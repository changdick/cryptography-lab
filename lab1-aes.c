...
/**
 * 将数组中的元素循环左移step个
 * 对矩阵行移位的时候，每一行依次调用这个函数，step传0，1，2，3， 所以可以根据step不同来移位
 */
void leftLoop4int(int array[4], int step)
{
    ...
    for (int i = 0; i < step; ++i) {
        ...
        temp = array[0];         // array[0] 先存着
        for (int j = 0; j < 3; ++j) {
            array[j] = array[j+1];            // array[0] 到array[2]依次用下一位赋值
        }
        array[3] = temp;         // temp是此前array[0],给到array[3]
...
//请补充代码
    ...
    for (int i = 0; i < step; ++i) {
        ...
        temp = array[3];         // array[0] 先存着
        for (int j = 3; j > 0; --j) {
            array[j] = array[j-1];            // array[3] 到array[1]依次用上一位赋值
        }
        array[0] = temp;         // temp是此前array[0],给到array[3]
...
void mixColumns(int array[4][4])
{
    ...
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            product = 0;
            for (int k = 0; k < 4; ++k) {
                product ^= GFMul(colM[i][k], temp[k][j]);          //用temp矩阵的值用来计算，实时更改array
            }
...
//请补充代码
    ...
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            product = 0;
            for (int k = 0; k < 4; ++k) {
                product ^= GFMul(deColM[i][k], temp[k][j]);          //用temp矩阵的值用来计算，实时更改array
            }
...
{
  //请补充代码 第 round 轮 使用 w[round * 4 + i]
  // w中一个元素是4个字节，经过函数转化成4个字，
    int roundKey[4];
    for (int i = 0; i < 4; ++i) {
        splitIntToArray(w[round * 4 + i] , roundKey); // roundKey 是 4个
        for (int j = 0; j < 4; ++j) {
...
//请补充代码
    // 字循环
    int splitword[4];
    splitIntToArray(num, splitword);  // 先将一个字num拆成4个字节，以进行循环
    leftLoop4int(splitword, 1);             // 循环左移一位
    // 对每个字节依次代换
    for (int i = 0; i < 4; ++i) {
        splitword[i] = getNumFromSBox(splitword[i]);
    }
    return mergeArrayToInt(splitword) ^ (int)Rcon[round];
}
